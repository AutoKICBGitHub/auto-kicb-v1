...
# Определяем возможные типы пользователей, счетов, филиалов, валют, статусы клиентов, и тарифов
USER_TYPES = ["INDIVIDUAL", "CORP", "MERCHANT"]
ACCOUNT_TYPES = ["CARD", "SETTLEMENT"]
BRANCHES = ["001", "533"]
CURRENCIES = ["USD", "SOM"]
CLIENT_TYPES = ["Bank Client", "External Client"]
...
def generate_sender_combinations():
    ...
    for user, account, branch, currency, client_type in product(USER_TYPES, ACCOUNT_TYPES, BRANCHES, CURRENCIES, CLIENT_TYPES):
        if (user, branch, account, currency, client_type) not in checked_senders:
            checked_senders.add((user, branch, account, currency, client_type))
            senders.append({
                'User From': user,
                'Account From': account,
                'Branch From': branch,
                'Currency From': currency,
                'Client Type From': client_type
            })
    return senders
...
def generate_receiver_combinations():
    ...
    for user, account, branch, currency, client_type in product(USER_TYPES, ACCOUNT_TYPES, BRANCHES, CURRENCIES, CLIENT_TYPES):
        if (user, branch, account, currency, client_type) not in checked_receivers:
            checked_receivers.add((user, branch, account, currency, client_type))
            receivers.append({
                'User To': user,
                'Account To': account,
                'Branch To': branch,
                'Currency To': currency,
                'Client Type To': client_type
            })
    return receivers
...
df_final.to_excel('C:/project_kicb/QR/final.xlsx', index=False)
