...
def make_statement_request(request, metadata):
...
def create_statement_request():
    ...
    statement_data = {
        ...
        "productType": "makeAccountStatementRequest",
        "data": {
    ...
    response = make_statement_request(request, metadata)
    print("Создание запроса выписки завершено")
    return response, statement_data

def confirm_statement_request(operation_id: str):
    metadata = (
        ('refid', str(uuid.uuid1())),
        ('sessionkey', get_session_data()['session_key']),
        ('device-type', 'ios'),
    ...
    response = make_statement_request(request, metadata)
    print("Подтверждение запроса выписки завершено")
    return response

def execute_statement_request():
    print("\n=== Начало создания запроса выписки ===")
    
    # Создаем запрос выписки
    statement_response, statement_data = create_statement_request()
    print(f"\nРезультат создания запроса: {statement_response}")
...
if __name__ == "__main__":
    try:
        result = execute_statement_request()
